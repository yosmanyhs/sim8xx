...
static 
#endif
size_t BtSppSendSerialize(void *p, char *obuf, size_t length)
{
...
static 
#endif
size_t BtSppSendParse(void *p, const char *ibuf, size_t length)
{
    BtSppSend_t *obj = (BtSppSend_t *)p;
    ...
    if (BT_SPP_SEND_STATE_DATA == obj->state) {
        if (n && ((AT_CMD_SEND_OK == status)         || 
                  (AT_CMD_SEND_FAIL == status)       ||
                  (AT_CMD_ERROR == status))) {
